...
### 3.2.1. 표기가 다른 단어들을 규칙에 의거 통합

> 같은 의미의 표기가 다른 단어들을 하나의 단어로 정규화

### 3.2.2. 대 소문자 통합

> 대문자를 소문자로 변환하는 소문자 변환작업
> 상황에따라 부분 대문자나 부분 소문자로 작업한다.

### 3.2.3. 불필요한 단어의 제거(Removing Unnecessary Words)

> 분석하고자 하는 목적에 맞지 않는 불필요 단어들을 노이즈 데이터
> 불용어 제거와 등장 빈도가 적은 단어, 길이가 짧은 단어들을 제거

#### 3.2.3.1. 등장 빈도가 적은 단어(Removing Rare words)

> 적게 등장해서 자연어 처리에 도움이 되지 않는 단어

#### 3.2.3.2. 길이가 짧은 단어(Removing words with a very short length)

> 길이를 조건으로 텍스트를 삭제하면서 단어가 아닌 구두점들까지도 한꺼번에 제거하기 위함(영어기준)
...
### 3.2.4. 정규 표현식(Regular Expression)

> 정규 표현식은 이러한 코퍼스 내에 계속해서 등장하는 글자들을 규칙에 기반하여 한 번에 제거하는 방식
...
> 컴퓨터가 이해할 수 있도록 자연어를 적절히 변환할 필요

### 11.1. 워드 임베딩

> 단어를 벡터로 표현하는 방법

#### 11.1.1. 희소표현(Sparse Representation)

> 벡터 또는 행렬(matrix)의 값이 대부분이 0으로 표현되는 방법
> 희소 벡터의 문제점은 단어의 개수가 늘어나면 벡터의 차원이 한없이 커져 공간적 낭비

#### 11.1.2. 밀집표현

> 희소 표현과 반대되는 표현이 밀집 표현
> 벡터의 차원이 조밀하게 차원이 증가 밀집 벡터(dense vector)라 한다.

#### 11.1.3. 워드 임베딩

> 단어를 밀집 벡터(dense vector)의 형태로 표현하는 방법을 워드 임베딩(word embedding)이라 합니다.
> 밀집 벡터를 워드 임베딩 과정을 통해 나온 결과라고 하여 임베딩 벡터(embedding vector)라고도 합니다.

### 11.2. 워드 투 벡터

> 단어 간 유사도를 반영할 수 있도록 단어의 의미를 벡터화하는 방법

### 11.3. 영/한 Word2Vec(실습)

### 11.4. Word2Vec 구현(실습)

### 11.5. 글로브(GloVe)

### 11.6. 패스트텍스트(FastText)

### 11.7. 자모단위 한국어 FastText학습하기
